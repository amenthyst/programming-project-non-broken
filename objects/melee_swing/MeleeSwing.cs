...
public partial class MeleeSwing : Area2D
{
        ...
        
    {
        ...
        rotation = directionVec.Angle() + Mathf.DegToRad(angle) / 2;
    ...
    {
        lifetimeTimer += delta;
        collisionPolygon2D.Polygon = make_sector_shape(Mathf.DegToRad(angle * (float)(lifetimeTimer / lifetime)), amount_of_segments, radius, rotation);
    ...
    private Vector2[] make_sector_shape(float angle, int amount_of_segments, float radius, float rotation)
    {
        ...
        for (int i = 1; i <= amount_of_segments; i++)
        {
            ...
            float angle_of_segment = rotation - (i / (float)amount_of_segments * angle);
